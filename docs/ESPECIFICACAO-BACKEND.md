...
| `searchText` | varchar(400) | sim | minúsculas, sem acento: `title toy collector brandName serieTitle` |
...
| **`signUp({ name, email, password })`** (novo) | `{ ok: true; user }` \| `{ ok: false; error: "email_in_use" \| "weak_password" \| "invalid_email" \| "rate_limited" \| "network" \| "unknown" }`. Já cria a sessão |
...
| Item | Valor | Situação |
|---|---|---|
| Postgres | host 179.199.139.63, porta 5433, banco `tosave`, schema `public` | O inventário foi feito com o usuário `postgres`, antes da troca pedida. Todas as sessões abrem com `default_transaction_read_only=on`, então o servidor recusa escrita. O novo `usuario_readonly` **falhou na autenticação** com a senha da nota. O `.env.local` já aponta para ele e o `postgres` não é mais usado |
