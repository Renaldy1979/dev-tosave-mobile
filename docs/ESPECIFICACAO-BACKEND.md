...
**Validação completa (`npm run validate`, 23/09/2026): 18/18 OK.** Cobre bcrypt `$2b# ToSave Backend — Especificação (Appwrite 1.8.1)
...
, cadastro, login no cliente, catálogo por cursor, `catalog_meta`, busca, `series.carCount`, `add`/`add`/`set 99`/`remove` pela Function (~350 ms quente), `user_stats`, bloqueio de escrita direta e `user-cleanup`. Os usuários de teste foram apagados.
